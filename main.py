...
try:
    ...
    df = pd.read_csv('NYC_taxi.csv', parse_dates=['pickup_datetime'], nrows=1000)
except:
    print("""
...
train_perr = mean_absolute_percentage_error(y_train, train_pred)

test_pred = model.predict(X_test)
test_rmse = np.sqrt(mean_squared_error(y_test, test_pred))
test_perr = mean_absolute_percentage_error(y_test, test_pred)




print("Train RMSE: {:0.2f}".format(train_rmse))
print("Train Percent Error: %.2f%%" % (train_perr))
print("Test RMSE: {:0.2f}".format(test_rmse))
print("Test Percent Error: %.2f%%" % (test_perr))
print('------------------------')
